...
// Server provides an HTTP-wrapped interface to the gRPC API of the delivery service
...
func (s *HttpWrappedGrpcServer) Run() error {
	log.Printf("[delivery-api grpc-web (HTTP)] starting on %v", serviceregistry.DeliveryServiceGrpcwebPort)
	...
	return http.ListenAndServe(fmt.Sprintf(":%d", serviceregistry.DeliveryServiceGrpcwebPort), handler)
}
